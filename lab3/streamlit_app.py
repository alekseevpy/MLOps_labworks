...
st.write(
    "Ниже перечислены 30 признаков, используемых в наборе данных sklearn.datasets.load_breast_cancer"
)
st.write(
    "Оригинальный набор данных взят с https://archive.ics.uci.edu/dataset/17/breast+cancer+wisconsin+diagnostic"
)
for label, example_value in fields.items():
    input_values[label] = st.number_input(
        label=label, value=example_value, step=0.00000001, format="%0.8f"
    )
